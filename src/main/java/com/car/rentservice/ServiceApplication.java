...
class DataInsertingCommandLineRunner implements CommandLineRunner {
	...
	@Autowired
	private MakeTagsRepository makeTagsRepository;
...
//	private TransmissionTagRepository  transmissionTagRepository;
	...
	public void run(String... args) throws Exception {
		...
		if (isDdl.equals("create") || isDdl.equals("create-drop")) {
			...
			makeTags.setMake("Mercedes");
			makeTagsRepository.save(makeTags);

			MakeTags makeTags2 = new MakeTags();
			makeTags2.setMake("Toyota");
			makeTagsRepository.save(makeTags2);

			PlaceNameTag placeNameTag = new PlaceNameTag();
			placeNameTag.setCity("Netanya");
			placeNameTagsRepository.save(placeNameTag);

			PlaceNameTag placeNameTag1 = new PlaceNameTag();
			placeNameTag1.setCity("Tel Aviv");
			placeNameTagsRepository.save(placeNameTag1);

			ModelTag modelTag = new ModelTag();
			modelTag.setModel("Mercedes Benz");
			modelTagRepository.save(modelTag);
			ModelTag modelTag2 = new ModelTag();
			modelTag2.setModel("Toyota corolla");
			modelTagRepository.save(modelTag2);

			YearTag yearTag = new YearTag();
			yearTag.setYear("2019");

			yearTagRepository.save(yearTag);
			YearTag yearTag2 = new YearTag();
			yearTag2.setYear("2015");
			yearTagRepository.save(yearTag2);


			WdTag wdTag = new WdTag();
			wdTag.setWd("RWD");
			wdTagRepository.save(wdTag);
			WdTag wdTag2 = new WdTag();
			wdTag2.setWd("4X4");
			wdTagRepository.save(wdTag2);


//			TransmissionTag transmissionTag = new TransmissionTag();
//			transmissionTag.setTransmission("Automatic");
//			TransmissionTag transmissionTag2 = new TransmissionTag();
//			transmissionTag2.setTransmission("Manual");

			HorsePowerTag horsePowerTag = new HorsePowerTag();
			horsePowerTag.setHorsepower("310hp");
			horsePowerTagRepository.save(horsePowerTag);
			HorsePowerTag horsePowerTag2 = new HorsePowerTag();
			horsePowerTag2.setHorsepower("220hp");
			horsePowerTagRepository.save(horsePowerTag2);

			FuelTag fuelTag = new FuelTag();
			fuelTag.setFuel("Gas");
			fuelTagRepository.save(fuelTag);
			FuelTag fuelTag2 = new FuelTag();
			fuelTag2.setFuel("Diesel");
			fuelTagRepository.save(fuelTag2);

			FuelConsumptionTag fuelConsumptionTag = new FuelConsumptionTag();
			fuelConsumptionTag.setFuelConsumption("8L/100Km");
			fuelConsumptionTagRepository.save(fuelConsumptionTag);
			FuelConsumptionTag fuelConsumptionTag2 = new FuelConsumptionTag();
			fuelConsumptionTag2.setFuelConsumption("10L/100km");
			fuelConsumptionTagRepository.save(fuelConsumptionTag2);

			EngineTag engineTag = new EngineTag();
			engineTag.setEngine("3000 V6");
			engineTagRepository.save(engineTag);
			EngineTag engineTag2 = new EngineTag();
			engineTag2.setEngine("2000/AC");
			engineTagRepository.save(engineTag2);
		}
	}
}
